...
namespace QuizApplication.Service.Implementation
{
    public class RoleService : IRoleService
    {
        ...
        public BaseResponseModel CreateRole(CreateRoleViewModel request)
        {
            ...
            var roleExist = _unitOfWork.Roles.Exists(r => r.ClassName == request.ClassName);

            if (roleExist)
            {
                response.Message = $"Role with name {request.ClassName} already exist!";
                return response;
            }
        ...
        public BaseResponseModel DeleteRole(string roleId)
        {
            ...
            if (role.ClassName == "Admin" || role.ClassName == "AppUser")
            {
